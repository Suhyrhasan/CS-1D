...
private:
	...
	DistObject myArray[7][7];	// Adjacency Matrix
	DistObject copy[7][7];
	string cityArray[7]; // Corresponding names
...
void Graph::DFS(int Xcoordinate, int Ycoordinate)
{
	...
	cout << "The total distance traveled on the discovery edges for the DFS: " << totalDistance << endl;
}
/******************************************************************************
...
void Graph::BFS(int xCd, int yCd)
{
	...
	cout << "The total distance traveled on the discovery edges for the BFS: " << totalDistance << endl;
}
/******************************************************************************
...
void Graph::DFSUtil(int Xcoordinate, int Ycoordinate)
{
	...
	else
	{
		...
		cout << setw(11) << left << cityArray[tempX];
		cout << setw(1)  << "--->";
		cout << setw(11) << left << cityArray[tempY];
		...
		cout << setw(6) << left << myArray[tempX][tempY].distance;
		cout << endl << flush;
...
void Graph::PrintAdjEdges(int xCd)
{
	...
	for(unsigned int i = 0; i < currVisit.size(); i++)
	{
		...
		//Output the node visited
		cout << setw(11) << left << cityArray[xCd] << " " << flush;
		cout << setw(1) << "-->";
		cout << setw(11) << left<< currVisit[i].city << " " << flush;

		//Output the distance for the trip
		cout << setw(6) << left << currVisit[i].distance << " "
				<< flush;
		cout << endl << flush;
...
{
	cout << "This is the adjacency matrix: \n";
	cout << "\t ";
	for(int index = 0; index < 7; index++)
	{
		cout << left;
		cout << setw(10) << cityArray[index];
	}
	cout << endl;
	for(int i=0; i<7; i++) //This loops on the rows.
	{
		...
		for(int j=0; j<7; j++) //This loops on the columns
		{
			if(myArray[i][j].distance == -1)
			{
				...
				cout << setw(10) << myArray[i][j].distance;
			}
		}
		cout << endl;
	}
